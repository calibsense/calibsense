...
"""caltrust's own JSON calibration format.

This is the format to emit when scripting against caltrust, and the one that
...
class NativeJsonReader(CalibrationReader):
    ...
    description: ClassVar[str] = "caltrust JSON calibration"

    def sniff(self, path: str, head: str) -> bool:
        """Match a JSON file that names the caltrust calibration format."""
        return os.path.splitext(path)[1].lower() == ".json" and FORMAT in head

    def read(self, path: str) -> CalibrationRecord:
        """Parse a caltrust JSON calibration.
...
def write_calibration(record: CalibrationRecord, path: str) -> str:
    """Write a calibration in caltrust's own JSON format.
