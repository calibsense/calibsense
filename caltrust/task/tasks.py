...
Where a task depends on something caltrust did not calibrate — a stereo baseline
...
class StereoTriangulation(Task):
    ...
    One camera at two places, or two identical cameras — either way caltrust
    ...
    The baseline is **treated as exact.** It is not something caltrust measured,
    ...
    def describe(self) -> str:
        ...
        return (
            ...
            "exact because caltrust did not measure it"
        )
...
class CameraToBase(Task):
    ...
    repeatability is a specification of the arm, not something caltrust
    ...
    def __post_init__(self) -> None:
        if self.hand_eye_result is None:
            raise ValidationError(
                ...
                "caltrust.handeye.solve_hand_eye first"
            )
        object.__setattr__(self, "depth_mm", _positive(self.depth_mm, "depth_mm"))
        object.__setattr__(
            self, "lateral_mm", (float(self.lateral_mm[0]), float(self.lateral_mm[1]))
