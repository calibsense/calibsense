        ...
        with np.load(path, allow_pickle=False) as archive:
            if _MANIFEST_KEY not in archive.files:
                raise SerializationError(
                    f"{path} is not a calibsense bundle: no manifest inside"
                )
...
def check_format(manifest: Mapping[str, Any], expected: str, max_version: int) -> None:
    ...
    if version > max_version:
        raise SerializationError(
            f"bundle format version {version} is newer than this calibsense "
            f"understands (max {max_version}); upgrade calibsense"
        )
