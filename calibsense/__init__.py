...
calibsense computes what that number cannot: the full parameter covariance, which
...
```python
from calibsense import session_from_images, instrument
...
`calibsense.ingest` covers M1: images with a known target, or a calibration file
...
`calibsense.refit` covers M2: the same calibration re-run with everything the
...
`calibsense.validate` covers M3: K-fold cross-validation over views, which turns
the in-sample reprojection RMS into an honest out-of-sample number and a ratio
between them.

`calibsense.diagnose` covers M4: one diagnostic per cause — pose diversity, depth
...
`calibsense.task` covers M5: the parameter covariance propagated by Monte Carlo
...
`calibsense.handeye` covers M6: the camera-to-robot transform with its own
covariance, and whether the pose set determines it at all.

`calibsense.report` covers M7: the whole audit as one paragraph in millimetres, a
JSON record, and a PDF, ending with what the audit could not settle.

`calibsense.noisefloor` measures corner noise directly from frames of a static
