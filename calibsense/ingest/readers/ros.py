...
#: ROS distortion model names mapped onto calibsense's two families.
_MODELS = {
    "plumb_bob": "pinhole",
    ...
    def read(self, path: str) -> CalibrationRecord:
        ...
        Raises:
            UnsupportedFormatError: The YAML will not parse, a required key is
                absent, or the distortion model is one calibsense cannot represent.
        """
        payload = _load_yaml(path)
        model_name = str(payload.get("distortion_model", "")).strip().lower()
        if model_name not in _MODELS:
            raise UnsupportedFormatError(
                f"{path} declares distortion_model {model_name!r}; calibsense "
