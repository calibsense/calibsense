...
#: Kalibr distortion model names mapped onto calibsense's two families.
_DISTORTION = {
    "radtan": "pinhole",
    "none": "pinhole",
    "equidistant": "fisheye",
    ...
    def read(self, path: str) -> CalibrationRecord:
        ...
        Raises:
            UnsupportedFormatError: The chain is empty, the requested camera is
                absent, or its model is one calibsense cannot represent.
        """
        ...
        if model != "pinhole":
            raise UnsupportedFormatError(
                f"{path}:{name} uses camera_model {model!r}; calibsense supports "
