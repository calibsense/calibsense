...
Everywhere else in calibsense the noise scale is inferred from residuals, which
...
That matters because the covariance in `calibsense.refit` estimates the noise
...
class NoiseFloor:
    ...
    def sigma_for_propagation(self) -> float:
        """The deviation to hand to `calibsense.task.propagate`.
...
def measure_noise_floor(
    ...
    Args:
        observations: One view per frame, all of the same static scene. Build it
            with `calibsense.ingest.detect_in_images` over the captured frames.
