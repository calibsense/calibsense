...
from ..errors import CalibSenseError
from ..ingest import (
    DetectorOptions,
    detect_in_images,
    find_images,
...
EXIT_UNEXPECTED = 1
EXIT_INPUT = 2
#: `calibsense diagnose` returns this when it finds a critical problem, so a CI
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="calibsense",
        description=(
            "Measurement uncertainty for camera calibration. Reports uncertainty, "
    ...
    calib.add_argument("--detections", required=True, metavar="FILE",
                       help="corner detections, as calibsense JSON or an npz")
    ...
    noise = subparsers.add_parser(
        ...
        description="Point a fixed camera at a fixed target, capture thirty or "
                    ...
                    "calibsense infers sigma from fit residuals, which assumes the "
...
def _diagnose(args: argparse.Namespace) -> int:
    ...
    try:
        fit = _fit_with_validation(session, options, args)
    except CalibSenseError as exc:
        print(f"calibsense: out-of-sample error not measured: {exc}", file=sys.stderr)
        fit = instrument(session, options)
...
def _report(args: argparse.Namespace) -> int:
    ...
    if needs_hand_eye:
        if audit.hand_eye is None:
            print(
                "calibsense: a base-frame task needs a hand-eye solve; pass "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not getattr(args, "handler", None):
        # A bare `calibsense`, or `calibsense ingest` with no source.
        (parser if args.command is None else parser).print_help()
        return EXIT_INPUT
    try:
        return args.handler(args)
    except CalibSenseError as exc:
        print(f"calibsense: {exc}", file=sys.stderr)
        return EXIT_INPUT
    except KeyboardInterrupt:
        print("calibsense: interrupted", file=sys.stderr)
