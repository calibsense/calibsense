...
# PyInstaller spec for a single-file calibsense binary.
#
# Build with `make binary`, which runs:
#     python -m PyInstaller --clean --noconfirm packaging/calibsense.spec
#
# calibsense is written to survive freezing. Every registry -- camera models,
...
# NumPy pull in transitively but calibsense never calls.
...
analysis = Analysis(
    [os.path.join(ROOT, "calibsense", "__main__.py")],
    pathex=[ROOT],
    binaries=[],
    datas=[],
    hiddenimports=[],
